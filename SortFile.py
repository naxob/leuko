...
def batch_sort(input, output, key=None, buffer_size=32000, tempdirs=None,rev=False):
    if tempdirs is None:
        tempdirs = []
    if not tempdirs:
        ...
        with open(input,'rb',64*1024) as input_file:
            ...
            for tempdir in cycle(tempdirs):
                ...
                if not current_chunk:
                    break               
                current_chunk.sort(key=key,reverse=rev)              
...
def main(filein=0,fileout=0,key=0,buffer=32000,rev=False):
    """
    import optparse
    ...
    """    
    key = eval('lambda line : (%s)'%key)
    print 'SortFile started sorting '+str(filein)+' to '+str(fileout)+' with key '+str(key)+' lines '+str(buffer)
    batch_sort(filein,fileout,key,buffer,None,rev)
